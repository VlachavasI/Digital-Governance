$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε το επίδομα!"
        : "We are sorry but you are not able to receive the benefit!";
  ...
  function retrieveAnswers() {
    ...
    if (allAnswers[1] === "1") {
      getEvidencesById(0);
    }else if(allAnswers[1] === "2"){
      getEvidencesById(7);
    }else if(allAnswers[1] === "3"){
      getEvidencesById(6);
    }else if(allAnswers[1] === "4"){
      getEvidencesById(6);
    }else if(allAnswers[1] === "5"){
      getEvidencesById(8);
    }else if(allAnswers[1] === "6"){
      getEvidencesById(8);
    }else if(allAnswers[1] === "7"){
      getEvidencesById(8);
    }else if(allAnswers[1] === "8"){
      getEvidencesById(8);
    }
    if (allAnswers[2] === "1") {
      getEvidencesById(8);
    ...
    if (allAnswers[6] === "1" || allAnswers[6] === "2" || allAnswers[6] === "3") {
      getEvidencesById(1);
    }
    if (allAnswers[7] === "1") {
      getEvidencesById(2);
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει να προσκομίσετε για να λάβετε το επίδομα είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive the allowance are the following:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Μπορείτε να κλαβετε το επίδομα μόνο μια φορά ανά έτος.") : skipToEnd("You can receive the benefit only once per year.");
        ...
        currentLanguage === "greek" ? skipToEnd("Πρέπει να είστε μόνιμος και νόμιμος κάτοικος της Ελλάδας.") : skipToEnd("You must be permanent and legal resident of Greece.");
          ...
          currentLanguage === "greek" ? skipToEnd("Πρέπει να είστε μόνιμος και νόμιμος κάτοικος της Ελλάδας.") : skipToEnd("You must be permanent and legal resident of Greece.");
            ...
            currentQuestion = -1;
            currentLanguage === "greek" ? skipToEnd("Πρέπει να έχετε άδεια διαμονής για τη χώρα.") : skipToEnd("You must hold a residence permit for the country.");
      } else if ((currentQuestion === 7 && selectedRadioButtonIndex === 2)&&(currentQuestion === 8 && selectedRadioButtonIndex === 2)&&(currentQuestion === 9 && selectedRadioButtonIndex === 2)&&(currentQuestion === 10 && selectedRadioButtonIndex === 2)) {
        currentQuestion = -1;
        currentLanguage === "greek" ? skipToEnd("Δεν δικαιούστε το επίδομα.") : skipToEnd("You are not eligible to receive the benefit.");
      } else {
